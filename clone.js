...
const { imageToWebp, imageToWebp3, videoToWebp, writeExifImg, writeExifImgAV, writeExifVid } = require('./start/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./start/lib/myfunction')
...
const jadibot = async (client, m, from) => {
if (Object.keys(client).includes(from)) {
    return client.sendMessage(from, { text: "Kamu sudah jadibot sebelumnya!" }, {quoted: m})
  }
const { state, saveCreds } = await useMultiFileAuthState(`./database/${m.sender.split("@")[0]}`)
try {
...
if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
     setTimeout(async () => {
       ...
       let txt = `*JADIBOT - CLONE*\nKode: *${code}*\n\n> Silakan buka WhatsApp, Perangkat Tertaut, Tautkan perangkat menggunakan kode, Lalu masukan kode.`
...
require('./message')(client[from], m, chatUpdate, mek, store)
}
} catch (err) {
console.log(err)
}
...
connectToWhatsApp();
  ...
  client.sendMessage(from, { text: `Bot Clone Terhubung...` }, {quoted: m});
  await sleep(3000)
...
async function stopjadibot(client, m, from) {
  if (!Object.keys(client).includes(from)) {
    return client.sendMessage(from, { text: `Kamu tidak ada di list jadibot!` }, {quoted: m})
  }
  try {
    client[from].end("Stop")
  } catch {}
  delete client[from]
  rimraf.sync(`./database/${from.split("@")[0]}`)
}
...
async function listjadibot(client, m) {
  ...
  return client.sendMessage(from, { text: text.trim(), mentions, }, {quoted: m})
}
